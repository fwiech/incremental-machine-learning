...
\hfill \break
These first discussion on AI ethics begun more frequently by technologists and policymakers, but the public opinion has not shaped much of these conversations \cite{governanceai_public_report}.
...
Or the integration of deep learning in edge devices limits the model on performenace and memory.
...
Neurala is a software based company which offer consulting services to their clients.
...
In the following months their clients approach Neurala again and asks them to integrate the new dataset to the previous that does not exist anymore.
\hfill \break
...
But because of ethical reasons there exist privacy laws that do not allow uploading kid's images, even related information to kids to the cloud.
...
Moreover the identifying of the kid has to be done in seconds.
...
Moreover continual learning is a key hurde to be able to develop an artificial general intelligence.
This term thinks of a machine intelligence that is able to perform any intellectual task that a human being is able to.
For this scientific topic exist multiple non-profit organisations and companies like OpenAI, Google DeepMind and the AGI Society, who delve this topic.
...
The challenge of continual learning and overcoming of catastrophic forgetting has been addressed in many studies:
...
In this research was the Elastic Weight consolidation reimplemented.
But since there is no assumption by the authors, why they use the Fisher Information Matrix, this article testes a alternative of calculating the EWC loss  function.

This article demonstrates that the EWC works just well or better with the alternative solution to the Fisher Information Matrix.
The are multiple benchmarks used based on the mnist dataset.
These benchmarks are segmented into szenarios.
We tested the szenarios of 9 base classes and the appendix of one, a base of 5 classes and appending 5 classes and the permutation of the mnist dataset.
