...
It is a great resource to test new deep learning algorithms with supervised learning.
...
The value of a neuron can be calculated like the following \cite{medium_nn_from_scratch}:
...
After a prediction by the calculation of all neurons and layers, the network needs a source of information on the progress of the learning process.
...
It is an algorithm which calculates error gradients with respect to each network variable. % TODO check if this is correct
...
% TODO deeper description (T_2 and complete dataset)
\begin{figure}[H]
    \centering
...
% TODO change figure (no complete dataset)
\begin{figure}[H]
    \centering
